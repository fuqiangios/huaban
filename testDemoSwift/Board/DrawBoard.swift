...
extension DrawBoard {
    ...
    fileprivate func DrawTextAndImage(){
        ...
        for lable in self.lableArray {
            att = [NSAttributedString.Key.foregroundColor.rawValue: lable.textColor,
                   NSAttributedString.Key.font: self.textFont,
                   NSAttributedString.Key.backgroundColor: UIColor.clear] as! [NSAttributedString.Key : Any]            //水印文字大小
...
extension DrawBoard {
    ...
    func initEventHendle() {
        ...
        CLNotificationCenter.addObserver(self, selector: #selector(keyBoardWillShow(_:)), name: UIResponder.keyboardWillShowNotification, object: nil)
        CLNotificationCenter.addObserver(self, selector: #selector(keyBoardWillHide(_:)), name: UIResponder.keyboardWillHideNotification, object: nil)
